...
//inverse変数
  inv_raw_image = malloc(sizeof(unsigned char*)*IMAGE_SIZE);
  ...
  for(i=0; i<IMAGE_SIZE; i++){
    ...
    inv_raw_image[i] = malloc(sizeof(unsigned char)*IMAGE_SIZE);
  ...
  while(block_number < 32*32){
    ...
    if(dc_flag){
      ...
      //dc_値分のビット進める
      inverse_binary_progress += (dc_group + 1);
      //acに移行
      dc_flag = false;
      element_number = 1;
    }
    ...
    //   //終わりビット判定
      ...
      if(end_block_flag){
        ...
        element_number = 63;
        end_block_flag = true;
        inverse_binary_progress += (64 - element_number);
      }

      //特殊ビット判定
      if(element_number != 63){
        for(i=0; i<ac_length_table[165]; i++){
          if(inversed_encoded_binary_array[inverse_binary_progress+i] == ac_code_table[165][i]){
        ...
        ac_group = ac_vlc%11;
        inverse_binary_progress += ac_length_table[ac_vlc];
  ...
  for(i=0; i<IMAGE_SIZE; i++){
    for (n=0; n<IMAGE_SIZE; n++){
      fread(&inv_raw_image[i][n], sizeof(unsigned char),1,fp);
    }
  }
  fclose(fp);

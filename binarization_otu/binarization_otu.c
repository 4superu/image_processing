...
int main(){
  ...
  int i, j, class_threshould, between_class_threshould, between_class_threshould_ver2;//ループ変数と２つの閾値
  int total_arry_element_left, total_arry_element_right;//それぞれのクラスでの画素数
  int *image_histgram;//画像を元に作ったヒストグラム
  float variance_left, variance_right, *class_variance_arry, *between_class_variance_arry, *between_class_variance_arry_ver2;//それぞれのクラスでの分散、２種類の分散を格納する配列
  float image_pixel_average, hist_average_left, hist_average_right;//全画素値平均,各クラスヒストグラムの平均値
  unsigned char *image_arry, *binary_image_arry_in_class, *binary_image_arry_between_class, *binary_image_arry_between_class_ver2;//二種類の閾値に基づいて作成する画像配列
  FILE *fp;
...
//クラス分けを行い、それぞれ計算する
  ...
  between_class_variance_arry_ver2 = (float*)malloc(sizeof(float)*255);
  for(i=0;i<255;i++){
    class_variance_arry[i] = 0;
    between_class_variance_arry[i] = 0;
    between_class_variance_arry_ver2[i] = 0;
  }
  for(i=0; i<255; i++){
    //左側クラスの計算
    ...
    between_class_variance_arry_ver2[i] = total_arry_element_left*pow((hist_average_left - image_pixel_average),2) + total_arry_element_right*pow((hist_average_right - image_pixel_average),2)/(total_arry_element_left + total_arry_element_right);
...
//クラス内分散とクラス間分散の最適値を取る閾値をそれぞれ出力
  ...
  between_class_threshould_ver2 = LargestBetweenClassVarianceThreshould(between_class_variance_arry_ver2);
//それぞれの閾値での分散値と最適な閾値を出力
  printf("θ値 クラス内分散和 クラス間分散\n");
  for(i=0;i<255;i++){
    printf("%3d  %5.6f  %5.6f\n",i,class_variance_arry[i],between_class_variance_arry[i]);
  }
  printf("\nクラス内分散和の最小値を取る閾値：%d　クラス間分散の最大値を取る閾値：%d クラス間分散の途中式ver：%d \n\n",class_threshould, between_class_threshould, between_class_threshould_ver2 );
...
//それぞれの閾値に基づき生成する画像
  ...
  binary_image_arry_between_class_ver2 = (unsigned char*)malloc(sizeof(unsigned char)*IMAGE_SIZE_LENNA*IMAGE_SIZE_LENNA);
